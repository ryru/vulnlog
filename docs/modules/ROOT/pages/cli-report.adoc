...
A vulnerability whose `resolution` targets a release outside this range is rendered under its pre-resolution state (`open` for `affected`, `dismissed` for `not affected` and for `affected` with `disposition: wont-fix`), so the report reflects what is actually shipped at the chosen release.
...
- xref:overview-report.adoc[Vulnlog Reporting]
