...
| `--verdict <value>`
| Triage verdict. One of `affected`, `not affected`.

| `--severity <value>`
| Severity. One of `low`, `medium`, `high`, `critical`.

| `--disposition <value>`
| Remediation intent for an `affected` verdict. One of `will-fix`, `wont-fix`.
...
When writing to a file, the whole file is rewritten in the canonical style, exactly as xref:reference/cli-fmt.adoc[fmt] would format it.
Any valid YAML layout is accepted as input.
YAML comments are not preserved; the command warns when it drops them.

The `--verdict`, `--severity`, `--disposition`, and `--justification` values are written exactly as given and are not cross-checked, so an inconsistent combination (for example `affected` together with a justification) is accepted here and reported later by xref:reference/cli-validate.adoc[validate].
