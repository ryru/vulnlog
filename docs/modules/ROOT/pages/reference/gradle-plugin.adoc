...
| `baseline`
| _(none)_
| A document you published before. Its identifier is kept and its version counts up.
|===
...
`baseline` must not be the `outputFile`: Gradle cannot read and write one file in a single task, and the build fails when both point at the same path.
To update a committed document in place, point `baseline` at the committed file and copy the task output over it, or run `vulnlog vex openvex --baseline <file> -o <file>` from the CLI.
