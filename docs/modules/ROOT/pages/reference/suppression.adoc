...
Whether the generated files are committed or created on the fly in CI is a workflow decision, covered in xref:guides/generate-suppression-files.adoc[Generate suppression files for your scanner].
...
`affected` findings (with or without a `wont-fix` disposition) and not-yet-triaged findings are still a live or unknown risk.
