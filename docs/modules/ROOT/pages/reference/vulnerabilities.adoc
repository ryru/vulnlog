= Vulnerabilities
:page-aliases: vulnerabilities.adoc
...
| String (enum)
| Conditional
| Severity assessment. Required when `verdict` is `affected`. See xref:reference/verdicts-and-justifications.adoc#severity-values[Severity Values].

| `verdict`
| String (enum)
| No
| The human triage decision. See xref:reference/verdicts-and-justifications.adoc[Verdicts and Justifications]. Absence indicates under investigation.

| `disposition`
| String (enum)
| No
| Remediation intent for an `affected` verdict: `will-fix` or `wont-fix`. See xref:reference/verdicts-and-justifications.adoc#disposition-values[Disposition Values].

| `justification`
| String (enum)
| Conditional
| Required when `verdict` is `not affected`. See xref:reference/verdicts-and-justifications.adoc#justification-values[Justification Values].
...
| String (enum)
| Yes
| The scanner or source type. See xref:reference/suppression.adoc#supported-reporters[Supported Reporters].
