...
| String (enum)
| Conditional
| Severity assessment. Required when `verdict` is `affected`. See xref:verdicts-and-justifications.adoc#severity-values[Severity Values].
...
| String (enum)
| No
| Remediation intent for an `affected` verdict: `will-fix` or `wont-fix`. See xref:verdicts-and-justifications.adoc#disposition-values[Disposition Values].
