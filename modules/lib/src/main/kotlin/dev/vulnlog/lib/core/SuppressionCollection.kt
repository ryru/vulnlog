...
fun collectSuppressedVulnerabilities(
    ...
    return SuppressionCollectionResult(
        ...
        exclusions = (resolved.map(::resolvedExclusion) + expired.mapNotNull(::expiredExclusion)).distinct(),
    )
}

private fun resolvedExclusion(vulnerability: VulnerabilityEntry): SuppressionExclusion =
    SuppressionExclusion.ResolvedVulnerability(vulnerability.id)
